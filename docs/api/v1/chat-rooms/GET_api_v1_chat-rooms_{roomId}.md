...
5. **Join Itinerary**: `chat_rooms.id = itineraries.room_id` 조건으로 연결된 일정을 조회합니다. `itineraries.room_id`는 UNIQUE이므로 정확히 1건이 존재합니다.
6. **Return**: 채팅방 메타 정보와 `itineraryId`를 함께 반환합니다.
...
```bash
curl -X GET https://your-api-domain.com/api/v1/chat-rooms/{roomId} \
