...
| `action_result` | JSONB | | AI 액션 결과 스냅샷. `assistant` 메시지에서 `itinerary`/`change`/`reservation`/`cancel` 타입 처리 시 저장. 그 외 `null` |
...
| `destinations` | JSONB | NOT NULL | 여행지 목록 (`DestinationItem` 배열: `[{"city": "...", "start_date": "...", "end_date": "..."}]`) |
...
| `destinations` | JSONB | | 변경 전 여행지 목록 스냅샷 |
